...
namespace SwFwLess\components\thrift;
 ...
 * @package thrift.transport
 */
class TCoroutineSocket extends TTransport
{
    /**
    ...
    {
        return $this->handle_ instanceof Socket;
    }
    ...
    public function open()
    {
        ...
        $this->handle_ = new Socket(AF_INET, SOCK_STREAM, 0);

        // Connect failed?
        if (!$this->handle_->connect(
            $this->host_,
            $this->port_,
            max($this->sendTimeoutSec_ + ($this->sendTimeoutUsec_ / 1000000), $this->recvTimeoutSec_ + ($this->recvTimeoutUsec_ / 1000000))
        )) {
    ...
    {
        if ($this->handle_ instanceof Socket) {
            @$this->handle_->close();
        }
        $this->handle_ = null;
    }
